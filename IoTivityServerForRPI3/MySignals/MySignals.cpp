...
void MySignalsClass::expanderWrite(byte data)
{
  ...
  #else
  int fd = wiringPiI2CSetup(EXPANDER);
...
byte MySignalsClass::expanderRead()
{
  ...
  #else  
  int fd = wiringPiI2CSetup(EXPANDER);
  return (byte)(wiringPiI2CRead(fd));
  #endif
}
...
int MySignalsClass::readADC(uint8_t adcnum)
{
  ...
  printf("start [%x] \n", MySignals.expanderState);
  MySignals.expanderWrite(MySignals.expanderState);

  digitalWrite(SD_CS, HIGH);                         //Disable SD SPI
  
  /*
  // strange A4 start signal start
  digitalWrite(10, LOW);
...
int MySignalsClass::readADC(uint8_t ADCPosition)
{
  ...
  printf("======>>\n");
  //MySignals.expanderWrite(0x10/*B00010000*/);
  ...
  //cli();
 printf("[0]%d, %x\n", ADCPosition, ((ADCPosition << 3) + 0b11000000));

 digitalWrite(10, LOW);
 delay(10);
  int value = (SPI.transfer((ADCPosition << 3) + 0b11000000) << 2);
  digitalWrite(10, HIGH);
  value = (value << 2);

  digitalWrite(10, LOW);
 delay(10);
  value += ((SPI.transfer(0) & 0b11110000) >> 4);
  digitalWrite(10, HIGH);
  ...
  // MySignals.expanderWrite(0x00/*B00000000*/);
  bitClear(MySignals.expanderState, EXP_ADC_CS);    //Disable ADC SPI
  MySignals.expanderWrite(MySignals.expanderState);
...
#if MYSIGNALS_DEBUG > 1
      printf("\n-----------------> SENSOR UART CONFIGURATION\n");
      printf("Glucometer UART enabled\n\n");
      // Serial.println();
      // Serial.println(F("-----------------> SENSOR UART CONFIGURATION"));
...
#if MYSIGNALS_DEBUG > 1
      printf("\n-----------------> SENSOR UART CONFIGURATION\n");
      printf("Blood pressure UART enabled\n\n");
      // Serial.println();
      // Serial.println(F("-----------------> SENSOR UART CONFIGURATION"));
...
#if MYSIGNALS_DEBUG > 1
      printf("\n-----------------> SENSOR UART CONFIGURATION\n");
      printf("Pulsioximeter UART enabled\n\n");
      // Serial.println();
      // Serial.println(F("-----------------> SENSOR UART CONFIGURATION"));
...
#if MYSIGNALS_DEBUG > 1
      printf("\n-----------------> SENSOR UART CONFIGURATION\n");
      printf("Pulsioximeter Micro UART enabled\n\n");
      // Serial.println();
      // Serial.println(F("-----------------> SENSOR UART CONFIGURATION"));
      // Serial.println(F("Pulsioximeter UART enabled"));
      // Serial.println();
...
#if MYSIGNALS_DEBUG > 1
      printf("\n-----------------> SENSOR UART CONFIGURATION\n");
      printf("Spirometer UART enabled\n\n");
      // Serial.println();
      // Serial.println(F("-----------------> SENSOR UART CONFIGURATION"));
...
#if MYHOSPITAL_DEBUG > 0
  printf("\n-----------------> Airflow SENSOR\n");
  printf("Calibrated ECG raw data = %f\n", sensorValue)
  // Serial.println();
  // Serial.println("-----------------> Airflow SENSOR");
...
#if MYHOSPITAL_DEBUG > 0
        printf("\n-----------------> ECG SENSOR\n");
        printf("BPM rate balanced = %d\n", airflowDataPPMBalanced);
        // Serial.println();
...
#if MYHOSPITAL_DEBUG > 0
  printf("\n-----------------> GSR SENSOR\n");
  printf("GSR conductance = %f\n", conductance);
  printf("GSR resistance = %f\n", resistance);
  printf("GSR voltage = %f\n", voltage);
  // Serial.println();
...
#if MYHOSPITAL_DEBUG > 0
  printf("\n-----------------> GSR SENSOR\n");
  printf("Calibrated GSR conductance = %f\n", conductance);
  printf("Calibrated GSR resistance = %f\n", resistance);
  printf("Calibrated GSR voltage = %f\n", voltage);
  // Serial.println();
  // Serial.println("-----------------> GSR SENSOR");
...
float MySignalsClass::getTemperature(void)
{
  ...
  printf("temperature(%f) resistance(%f) voltage(%f)\n", temperature, resistance, voltage);
#if MYHOSPITAL_DEBUG > 0
  printf("\n-----------------> TEMPERATURE SENSOR\n")
 ...
 */
  ///////////////
  Serial.write((char*)m1, sizeof(m1));
  Serial.write(0xA1);
  for (int i = 0; i < 6; i++)
  {
    Serial.write(0x80);
