...
#define pgm_read_word(address_short) address_short //__LPM_word_classic__((uint16_t)(address_short))
...
uint8_t BLEClass::softwareReset()
{
	...
	// delay till reset
	delay(500);
	//memset(answer, 0x00, MAX_PACKET_SIZE);
	...
	{	
		...
		{			
			...
			{										
				...
				#if BLE_DEBUG > 1
					MySignals.print("ev:");
					MySignals.PrintHex8(event, event[1]+4);
...
uint16_t BLEClass::readLocalAttribute(uint16_t handle, uint16_t offset)
{
	...
	MySignals.PrintHex8(attributeValue, aux);
	//for(i = 0; i < aux; i++)
	//{
...
{	
	...
	for(i = 0; i < length; i++)
	{		
		MySignals.PrintHex8(&payload[i+4], 1);
	}
	MySignals.println();
	#endif
...
uint16_t BLEClass::sendCommand(uint8_t LL, uint8_t CID, uint8_t CMD, uint8_t * PL)
{
	...
	#if BLE_DEBUG>1
	MySignals.print("Cmd: ");
	MySignals.PrintHex8(command, index);
	
	digitalWrite(ENABLE, LOW);
	printf("index:%d", index);
	// MySignals.print("index:");
	// MySignals.print(index,DEC);
	digitalWrite(ENABLE, LOW);
    ...
    // }
	Serial.write((char*)&(command[0]), index);
...
{	
	...
	*/
	...
	Serial.write((char *)customCommand, sizeof((char*)customCommand));
...
{	
	...
	// print answer
	MySignals.print("Ans: ");
	MySignals.PrintHex8(answer, i);
	MySignals.print(" ec:");
	MySignals.PrintHex16(&errorCode, 1);
	MySignals.println();
	delay(10);
	#endif
