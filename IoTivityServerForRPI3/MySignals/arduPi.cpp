...
SerialPi::SerialPi(){
	REV = getBoardRev();
    //printf("R1>%d\n", REV);
    serialPort="/dev/ttyAMA0";//"/dev/ttyS0";//
    timeOut = 1000;
}

#define USER_WIRINGPI 0
//Sets the data rate in bits per second (baud) for serial data transmission
void SerialPi::begin(int serialSpeed){
    if(fd > 0 || sd > 0) {
        printf("already opened. so close![%d,%d]\n", fd, sd);
        end();        
    }
#if USER_WIRINGPI
  fd = serialOpen(serialPort, serialSpeed);
  sd = fd;
  printf("Serial.begin=>%d, %s, %d\n", fd, serialPort, serialSpeed);
#else
	switch(serialSpeed){
...
int raspberryPinNumber(int arduinoPin){
	switch(arduinoPin){
		...
		case 8: return 21;/*if(REV == 1){return 21;}else{return 27;} */break;
