...
char *gClockResourceType= (char *)"oic.r.clock";
    ...
    OCEntityHandlerResponse *response, OCRepPayload **payload)
{
    ...
    if (_payload)
    {
...
#if 0
    	OIC_LOG(INFO, TAG, "POST DATA CHECK START!");
        ...
        {
        	OIC_LOG_V(INFO, TAG, "POST DATETIME[%s]", dateTimeStr);
//            printf("POST DATETIME[%s]\n", dateTimeStr);
        ...
        {
        	OIC_LOG_V(INFO, TAG, "POST COUNTDOWN[%f]", _countdown);
//            printf("POST COUNTDOWN[%f]\n", _countdown);
        }
        OIC_LOG(INFO, TAG, "POST DATA CHECK END!");
